...
public class CustomCalendar extends LinearLayout {
    ...
    private void invalidAttributes(String message) {
        ...
        //switch_data.setVisibility(INVISIBLE);
    }
    ...
    public void setDateSelectionData(ArrayList<EventData> dateData) {
         //switch_data = (Switch) findViewById(R.id.switch_novo_inicio);
        switch_data.setVisibility(View.INVISIBLE);
        ...
        if (dateData.size() > 0) {
            ...
            Date date = null;
            try {
                date = sdf.parse(Singleton.getInstance().getCurrentDate());
            } catch (ParseException e) {
                e.printStackTrace();
            }
            ...
            data_global = out.format(date);
            Log.d("DATA_COMO_QUERO", data_global);
            //Log.d("DATA_COMO_QUERO", CalendarioActivity.global);
            /*if(CustomCalendar.data_global.toString().equals(CalendarioActivity.global)){
            ...
            for (int i = 0; i < dateData.size(); i++) {
                ...
                if (dateData.size() > 0) {
                    //FAZER TOAST AQUIIIIIIIIIIIIIIIII
       ...
       // switch_data.setVisibility(View.VISIBLE);
        if (items.size() == 0) {
            ...
            try {
                SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
                Date date = sdf.parse(Singleton.getInstance().getCurrentDate());
                date.setDate(date.getDate());
                //Toast.makeText(getBaseContext(), "DATA!" + date, Toast.LENGTH_LONG).show();
                SimpleDateFormat out = new SimpleDateFormat("yyyy-MM-dd");
                data_global = out.format(date);
